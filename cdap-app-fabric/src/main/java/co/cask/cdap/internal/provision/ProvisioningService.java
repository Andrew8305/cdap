...
public class ProvisioningService extends AbstractIdleService {
  ...
  Runnable deprovision(ProgramRunId programRunId, DatasetContext datasetContext,
    ...
    if (existing == null) {
      LOG.error("Received request to deprovision a cluster for program run {}, but could not find information " +
    ...
    if (provisioner == null) {
      LOG.error("Could not deprovision cluster for program run {} because provisioner {} does not exist.",
